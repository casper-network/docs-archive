...
study this sub-problem in isolation. After having the sub-problem studied
enough, we apply the sub-problem in the wider context, i.e. building a
blockchain.
...
to Scala syntax, but was modified so that non-scala developers will be able to
grasp the meaning.
...
Every validator has a weight which is a non-zero integer value and is thought of as “the voting power of this validator”. In the context of proof-of-stake blockchains this corresponds to the bonded stake.
...
same pattern for estimators construction. The assumption is that the set of
